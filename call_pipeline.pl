...
my $pipeline_config_file = "/hpf/largeprojects/pray/clinical/config_test/v5_pipeline_config.txt"; #Future will be passed from the thing1 cmd
...
&check_opts;
&print_time_stamp;
my ($SCRIPTDIR, $ANNOVAR, $BACKUP_BASEDIR, $GATK, $BWA, $PICARDTOOLS, $SAMTOOLS, $TABIX, $PERL, $VCFTOOLS, $BEDTOOLS, $RSCRIPT, $reference, $dbSNP, $omni_vcf, $g1k_snp_vcf, $g1k_indel_vcf, $clinvar_indel_vcf, $hgmdAML, $hgmdAS, $hapmap_vcf, $vcfPaddingFile, $JAVA) = &read_in_pipeline_config($pipeline_config_file); #read in pipeline configuration
...
sub bwa_mem_s {
  ...
  for (1..$filenum) {
    ...
    . "$JAVA -jar -Djava.io.tmpdir=\$TMPDIR -Xmx6G \$PICARD/SortSam.jar INPUT=/dev/stdin OUTPUT=$runfolder/bwaAlign/picard.$_.sorted.bam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate TMP_DIR=\$TMPDIR && \\\n";
  }
  my $cmd = 'echo \''
    . 'export TMPDIR=/localhd/`echo $PBS_JOBID | cut -d. -f1 ` &&' . " \\\n"
    . "\\\n"
...
sub picardMarkDup {
  ...
  my $inputfiles = '';
  for (1..$filenum) {
    $inputfiles .= " I=$runfolder/$Pfolder/picard.$_.sorted.bam";
  }
  my $cmd = "";
  #    if ($fastqDir =~ /0000000/) {
  #        $cmd = 'echo \''
  ...
  $cmd = 'echo \''
    . 'export TMPDIR=/localhd/`echo $PBS_JOBID | cut -d. -f1 ` &&' . " \\\n"
    . "\\\n"
    . "$JAVA -jar -Djava.io.tmpdir=\$TMPDIR -Xmx36G $PICARDTOOLS  MarkDuplicates $inputfiles REMOVE_DUPLICATES=false CREATE_INDEX=true ASSUME_SORTED=true O=$runfolder/picardMarkDup/$sampleID.$postprocID.picard.sort.merged.rmdup.bam  METRICS_FILE=$runfolder/picardMarkDup/$sampleID.$postprocID.picard.sort.merged.rmdup.bam.metric_file &&" . " \\\n"
...
sub gatkLocalRealign {
  ...
  my $cmd = 'echo \''
    ...
    . 'module load ' . $SAMTOOLS . ' && ' . " \\\n"
    . "\\\n"
    . "$JAVA -jar -Djava.io.tmpdir=\$TMPDIR -Xmx34G \$GATK -T RealignerTargetCreator -I $runfolder/$Pfolder -o $runfolder/gatkLocalRealign/forRealigner.intervals -R $reference -l INFO &&" . " \\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx34G \$GATK -T IndelRealigner -I $runfolder/$Pfolder -o $runfolder/gatkLocalRealign/$sampleID.$postprocID.realigned.rmduped.mapq.bam -R $reference -l INFO -targetIntervals  $runfolder/gatkLocalRealign/forRealigner.intervals -compress 0 $maxReadsForRealignment &&" . " \\\n"
    . 'samtools index' . " $runfolder/gatkLocalRealign/$sampleID.$postprocID.realigned.rmduped.mapq.bam ; "
    . "\'| jsub -j gatkLocalRealign -b $runfolder  -nm 40000 -np 1 -nn 1 -nw 06:00:00 -ng localhd:100 $depend";
  print "\n\n************\ngatkLocalRealign:\n$cmd\n************\n\n";
  my $cmdOut = `$cmd`;
  print "============\n$cmdOut============\n\n";
  if ($cmdOut =~ /^(\d+)\n/) {
...
sub gatkCovCalExomeTargets {
  ...
  my $cmd = 'echo \''
    ...
    . 'module load ' . $PERL . " && \\\n"
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx14G \$GATK -T DepthOfCoverage --printBaseCounts --includeRefNSites --minMappingQuality 20 --minBaseQuality 20 " . $depthct
    ...
    . " \\\n"
    . "\'| jsub -j gatkCovCalExomeTargets -b $runfolder  -nm 20000 -np 1 -nn 1 -nw 06:00:00 -ng localhd:1 $depend";
...
sub gatkCovCalGP {
  ...
  my $cmd = 'echo \''
    ...
    . "\'| jsub -j gatkCovCalGP -b $runfolder  -nm 20000 -np 1 -nn 1 -nw 06:00:00 -ng localhd:1 $depend";
  print "\n\n************\ngatkCovCalExomeGP:\n$cmd\n************\n\n";
  my $cmdOut = `$cmd`;
  print "============\n$cmdOut============\n\n";
  if ($cmdOut =~ /^(\d+)\n/) {
...
sub gatkRawVariants {
  ...
  my $cmd = 'echo \''
    ...
    . 'module load ' . $GATK . ' && ' . " \\\n"
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx14G \$GATK -T CombineVariants --printComplexMerges -genotypeMergeOptions PRIORITIZE -l INFO \\\n"
    ...
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx14G \$GATK -T SelectVariants -selectType SNP  \\\n"
    ...
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx14G \$GATK -T SelectVariants -selectType INDEL \\\n"
    ...
    . "\'| jsub -j gatkRawVariants -b $runfolder  -nm 20000 -np 1 -nn 1 -nw 04:00:00 -ng localhd:10 $depend";
  print "\n\n************\ngatkRawVariants:\n$cmd\n************\n\n";
  my $cmdOut = `$cmd`;
  print "============\n$cmdOut============\n\n";
  if ($cmdOut =~ /^(\d+)\n/) {
...
sub gatkFilteredRecalSNP {
  ...
  my $cmd = 'echo \''
    ...
    . 'module load ' . $GATK . ' && ' . " \\\n"
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx34G \$GATK -T VariantRecalibrator -mode SNP -an DP -an QD -an FS -an MQRankSum -an ReadPosRankSum  \\\n"
...
sub gatkFilteredRecalINDEL {
  ...
  my $cmd = 'echo \''
    ...
    . 'module load ' . $GATK . ' && ' . " \\\n"
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx34G \$GATK -T VariantRecalibrator -mode INDEL -an DP -an FS -an MQRankSum -an ReadPosRankSum  \\\n"
...
sub gatkFilteredRecalVariant {
  ...
  my $cmd = 'echo \''
    ...
    . 'module load ' . $PERL . ' &&' . " \\\n"
    . "\\\n"
    . "$JAVA  -jar -Djava.io.tmpdir=\$TMPDIR -Xmx14G \$GATK -T CombineVariants --printComplexMerges -genotypeMergeOptions PRIORITIZE -priority recal_indels,recal_snps -l INFO  \\\n"
    ...
    . "\'| jsub -j gatkFilteredRecalVariant -b $runfolder  -nm 20000 -np 1 -nn 1 -nw 03:00:00 -ng localhd:10 $depend";
...
sub annovarMutect {
  ...
  my $cmd = 'echo \''
    ...
    . "\\\n"
    . 'sed -i "s/\tREJECT\t/\t0\t/;s/\tKEEP\t/\t1\t/;s/\tNA/\t/g;s/\tTRUE/\t1/g;s/\tFALSE/\t0/g" ' . " $runfolder/annovarMutect/$sampleID.$postprocID.csv"
    . "\\\n"
...
sub read_in_pipeline_config {
  ...
  my ($pConfigFile) = @_;
  my $data = "";
  my ($sd, $ann, $backup, $gatk, $bwa, $ptools, $stools, $tab, $pl, $vtools, $btools, $rscript,$reference, $dbSNP, $omni_vcf, $g1k_snp_vcf, $g1k_indel_vcf, $clinvar_indel_vcf, $hgmdAML, $hgmdAS, $hapmap_vcf, $vcfPaddingFile,$JAVA);
